...
// পপআপের মূল ডিজাইন (UI)
const CustomModal = () => {
    const { closePopup, content } = usePopup();

    return (
       <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 md:p-6">
    <div className="bg-white rounded-[2.5rem] max-w-5xl w-full p-6 md:p-10 shadow-2xl relative animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
        
        {/* Close Button */}
        <button
            onClick={closePopup}
            className="absolute top-6 right-6 p-2 bg-gray-100 text-gray-500 hover:bg-red-100 hover:text-red-600 rounded-full transition-all z-10"
        >
                ...
                <div className="bg-white border-2 border-gray-100 rounded-[2rem] p-6 hover:border-red-200 transition-all shadow-sm">
                    <div className="flex items-center gap-4 mb-4">
                        <div className="flex -space-x-3">
                            <img src="https://www.afsarsquest.com/_next/image?url=%2FbKash.png&w=64&q=75" alt="bKash" className="w-12 h-12 rounded-full border-4 border-white shadow-md" />
                            <img src="https://www.afsarsquest.com/_next/image?url=%2Fnagad-logo.png&w=48&q=75" alt="Nagad" className="w-12 h-12 rounded-full border-4 border-white shadow-md" />
                        </div>
                        <div>
